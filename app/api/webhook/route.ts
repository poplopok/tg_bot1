...
import { getChatStats, updateMessageStats } from "@/lib/chatStats"
...
async function analyzeEmotionLocal(text: string): Promise<EmotionAnalysis> {
  ...
  aggressionWords.forEach((word) => {
    if (lowerText.includes(word)) {
      ...
      console.log(`[LOCAL] Найдено агрессивное слово: "${word}" в тексте: "${text}"`)
    }
  })
...
bot.on("message:text", async (ctx) => {
  ...
  console.log(`[DEBUG] Получено сообщение: "${text}" от пользователя ${username} в чате ${chatId}`)
  ...
  try {
    ...
    console.log(`[WEBHOOK] Инцидент: ${isIncident}, Серьезность: ${analysis.severity}`)

    // Обновляем локальную статистику
    const stats = updateMessageStats(chatId, userId!, username, analysis.emotion, isIncident, analysis.severity, text)
    ...
    if (isIncident) {
      console.log(`[WEBHOOK] Добавлен инцидент для пользователя ${userId}`)
